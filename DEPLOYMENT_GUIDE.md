...
1. **Push the repo** to GitHub (this repo has `vercel.json` with `outputDirectory: "public"` so Vercel serves the built app).
2. **Vercel:** New Project → Import the GitHub repo. Use **Build Command** `npm run build`, **Output Directory** `public` (or leave default — `vercel.json` sets it). Deploy.
3. **Verify:** Open the deployment URL. You should see the full site (hero, nav, footer). If you see "This page is taking longer than usual," the deploy is still using the wrong output — confirm in Vercel project **Settings → General** that Output Directory is `public` and redeploy.
...
2. **Configure Build Settings:**
   ...
   - Output Directory: `public` (also set in `vercel.json` as `outputDirectory` so deploys use the built app; required for the site to load instead of showing "Loading…")
...
3. **If you see “Loading…” for more than a few seconds**  
   ...
   - In Vercel: set **Output Directory** to **`public`** (Vite writes `index.html` and `assets/` here). Leave **Root Directory** blank so the build runs from the repo root. Redeploy.  
